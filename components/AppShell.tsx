...
type Tab = "minimal" | "envelopes" | "cash";
    ...
    <div>
      ...
      >
        <div className="flex gap-1 rounded-full p-1" style={{ background: "var(--hover)", border: "1px solid var(--border)" }}>
          {(["minimal", "envelopes", "cash"] as Tab[]).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
            ...
            >
              {t === "minimal" ? "Главное" : t === "envelopes" ? "Конверты" : "Касса"}
            </button>
          ))}
        </div>
        <button
      ...
      {tab === "minimal" ? (
        ...
        <CashTab state={state} refresh={refresh} showToast={showToast} />
      )}
